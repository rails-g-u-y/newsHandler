...
module NewsreportsRails
  class NewsreportsTemplate < SimpleDelegator  
    attr_accessor :_generate_options

    def initialize(newsreports_report_base_obj, template_context, template_virtual_path)
      @_template_context = template_context
      @_template_virtual_path = template_virtual_path
      super(newsreports_report_base_obj)
    end
      ...
      %{
        if defined?(report)
          ...
          ThinReports::Report.create do |__report__|
            report = NewsreportsRails::NewsreportsTemplate.new(__report__, self, '#{template.virtual_path}') 
...
ActionView::Template.register_template_handler :newsreports, NewsreportsRails::TemplateHandler
